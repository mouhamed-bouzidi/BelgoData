...
export default function UsersPage() {
  ...
  async function handleRoleChange(userId: string, newRole: User["role"]) {
    ...
    try {
      await axios.put(`${API_URL}/api/auth/users/${userId}`, { role: newRole }, getAuthConfig());
  ...
  return (
    <div className="w-full max-w-full overflow-x-hidden p-4 sm:p-6 lg:p-8 bg-gradient-to-br from-violet-50/40 via-white to-slate-50/50 min-h-screen space-y-6 sm:space-y-8 animate-fade-in">
      {/* HEADER */}
      <div className="relative">
        ...
        <h1 className="relative text-2xl sm:text-3xl font-black tracking-tight bg-gradient-to-r from-purple-800 via-purple-600 to-fuchsia-600 bg-clip-text text-transparent">
          Gestion des Utilisateurs
        </h1>
        <p className="relative text-xs sm:text-sm font-medium text-slate-500 mt-1.5">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:hidden">
        {users.map((u) => (
          <div
            key={u._id}
            className="bg-white/85 backdrop-blur-sm border border-slate-200/60 rounded-2xl p-4 shadow-[0_4px_20px_-10px_rgba(109,94,240,0.15)] space-y-3"
          >
            <div className="flex items-start gap-3">
              <div className="w-10 h-10 shrink-0 rounded-2xl overflow-hidden flex items-center justify-center bg-gradient-to-br from-[#6d5ef0] via-[#7c6ef2] to-[#8b5cf6] text-white ring-2 ring-white shadow-[0_4px_12px_-2px_rgba(109,94,240,0.4)]">
                {u.avatarUrl ? (
                  <img src={u.avatarUrl} alt={`Avatar ${u.name}`} className="w-full h-full object-cover" />
                ) : (
                  <span className="font-bold text-sm">{u.name ? u.name.slice(0, 2).toUpperCase() : "??"}</span>
                )}
              </div>
              ...
              {u.status === "Actif" ? (
                <span className="shrink-0 inline-flex items-center gap-1 text-emerald-700 font-semibold text-[11px] bg-emerald-50/70 border border-emerald-100 px-2 py-1 rounded-full">
                  <CheckCircle2 size={12} /> Actif
                </span>
              ) : (
                <span className="shrink-0 inline-flex items-center gap-1 text-rose-600 font-semibold text-[11px] bg-rose-50/70 border border-rose-100 px-2 py-1 rounded-full">
            ...
            <div className="space-y-1">
              <div className="flex items-start gap-1.5 text-slate-600 text-xs font-medium">
                <Mail size={12} className="text-violet-400 mt-0.5 shrink-0" />
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
              <select
                ...
                className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold border shadow-sm cursor-pointer outline-none bg-transparent transition-all duration-200 focus:ring-2 focus:ring-violet-200 disabled:opacity-50 ${
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 text-slate-400 hover:text-[#6d5ef0] hover:bg-violet-50 rounded-xl transition-all"
                  title="Voir le dashboard de prospection"
                  aria-label="Voir le dashboard de prospection"
                >
                  <BarChart3 size={16} />
                </button>
                <button
      ...
      {/* VUE DESKTOP : tableau */}
      <div className="hidden lg:block bg-white/80 backdrop-blur-sm border border-slate-200/60 rounded-3xl shadow-[0_4px_24px_-8px_rgba(109,94,240,0.08)] overflow-hidden transition-all duration-300 hover:shadow-[0_8px_32px_-8px_rgba(109,94,240,0.15)]">
                ...
                <tr key={u._id} className="hover:bg-violet-50/30 transition-all duration-200 group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="font-bold text-slate-800 group-hover:text-[#6d5ef0] transition-colors truncate">
                          {u.name}
                        </div>
                  ...
                  <td className="px-6 py-4">
                    <select
                      ...
                      disabled={actionLoadingId === u._id}
                      onChange={(e) => handleRoleChange(u._id, e.target.value as User["role"])}
                      className={`px-2.5 py-1 rounded-lg text-xs font-semibold border shadow-sm cursor-pointer outline-none bg-transparent transition-all duration-200 focus:ring-2 focus:ring-violet-200 hover:brightness-95 disabled:opacity-50 ${
                        ROLE_COLORS[u.role] || ROLE_COLORS.Viewer
                      }`}
                    >
                      <option value="Viewer">Viewer</option>
                  ...
                  <td className="px-6 py-4">
                    {u.status === "Actif" ? (
                      <div className="inline-flex items-center gap-1.5 text-emerald-700 font-semibold text-xs bg-emerald-50/70 border border-emerald-100 px-2 py-1 rounded-full whitespace-nowrap">
                        <CheckCircle2 size={14} />
                        <span>Actif</span>
                      </div>
                    ) : (
                      <div className="inline-flex items-center gap-1.5 text-rose-600 font-semibold text-xs bg-rose-50/70 border border-rose-100 px-2 py-1 rounded-full whitespace-nowrap">
                        <XCircle size={14} />
                        <span>Inactif</span>
                      </div>
                    )}
